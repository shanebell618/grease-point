...
`src/features/equipment/` almost exactly.
...
- `src/features/maintenance/schema.ts`, `types.ts`, `api.ts`, `queryKeys.ts`, `hooks.ts`
  — copy `src/features/equipment/`'s versions and adjust fields.
- `src/features/maintenance/components/MaintenanceTable.tsx` — the "Maintenance
...
- A `MaintenanceHistory` section embedded in `EquipmentDetail.tsx`
  (`src/features/equipment/components/EquipmentDetail.tsx`) is a natural
...
`src/features/equipment/utils.ts`.
