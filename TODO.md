...
- [ ] **Stop repeating `<Controller>` per field in every form** — `EquipmentForm`, `MaintenanceForm`, and `PartForm` each hand-write a `<Controller name="x" control={control} render={({ field }) => <TextField {...field} .../>}>` block per field, inline, in the form itself — the exact same boilerplate duplicated across every text field in every form. Checked `dip-financing-portal`'s actual fix rather than guessing: it's not "less `Controller`," it's _where_ `Controller` lives. Two pieces: (1) `src/common/hooks/useMuiForm/` wraps RHF's `useForm` and hands back two field-prop generators instead of raw `register`/`control` — `registeredFieldProps(name, options)` for plain fields (just remaps RHF's `ref` to MUI's `inputRef`, no `Controller` needed at all), and `controlledFieldProps(name, options)` for fields that need one, returning `{ name, control, error, helperText, required }` as a ready-to-spread prop bundle. (2) Each _reusable input component_ that needs controlled behavior (see `MaskedPhoneField`, `ReactHookFormSelectField`) wraps `Controller` internally, exactly once, via a shared `ReactHookFormController` component (`src/common/components/ui/Input/ReactHookFormController/`) — so a form itself never writes `<Controller>` at all, it just does `<MaskedPhoneField {...controlledFieldProps("phone", { required: true })} />`. The `Controller` boilerplate moves from "once per field per form" to "once per reusable input component, ever." Bigger lift than it sounds — needs the `useMuiForm`-equivalent hook plus wrapping every existing `TextField`/`Select`/`DatePicker` usage as its own component — not something to fold into `PartForm`, which should keep matching the existing `EquipmentForm`/`MaintenanceForm` pattern for now so all three stay consistent with each other until this lands.

- [ ] **Adopt `next-safe-action` for writes** — `dip-financing-portal`'s `server/actions/*` files start with `"use server"` and are real Next.js Server Actions, wrapped by `next-safe-action`'s `actionClient` (`src/lib/safeAction.ts`, just `createSafeActionClient()`); a client component calls one directly via `useAction()`/`useAction().executeAsync()` from `next-safe-action/hooks`, which hands back `execute`/`isExecuting`/`result` for free — see `ProfileReview`/`useCompleteAction` (`src/verticals/auth/pages/BorrowerOnboardingPageView/`). Grease-point's `actions/*` files (`createPartAction.ts` and its Equipment/Maintenance equivalents) are plain async functions with no `"use server"` — they're not real Server Actions, they only run because an `app/api/**/route.ts` Route Handler calls them, and the client reaches them through hand-written `fetch()` in each vertical's `api.ts`, wrapped in TanStack Query's `useMutation`. The read side genuinely matches DIP (`dataAccess` → `useCases` → `actions` → route, reads skipping the action layer); the write side is a from-scratch REST-ish shim instead of DIP's actual pattern. `next-safe-action` isn't installed here. Fixing this touches every vertical at once — each `actions/*.ts` becomes a real `"use server"` file, `src/lib/safeAction.ts` gets added, the POST/PATCH/DELETE handlers come out of every `route.ts` (GET stays, since reads bypass actions either way), and every mutation hook (`useCreateEquipmentMutation` and friends) gets rewritten from `useMutation`+fetch to `useAction` — so it's a dedicated migration, not something to fold into whichever feature is in progress when it's noticed.
